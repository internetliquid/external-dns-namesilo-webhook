...
// ClientMetrics is the Prometheus implementation of the namesilo client's
...
func NewClientMetrics(reg prometheus.Registerer) *ClientMetrics {
	m := &ClientMetrics{
		apiRequests: prometheus.NewCounterVec(prometheus.CounterOpts{
			Namespace: "namesilo_webhook",
			Subsystem: "api",
			Name:      "requests_total",
			Help:      "Namesilo API requests, labelled by operation and result (success|error).",
		}, []string{"operation", "result"}),
		apiDuration: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			Namespace: "namesilo_webhook",
			Subsystem: "api",
			Name:      "request_duration_seconds",
			Help:      "Namesilo API request duration in seconds, labelled by operation.",
		...
		cacheHits: prometheus.NewCounter(prometheus.CounterOpts{
			...
			Subsystem: "cache",
			Name:      "hits_total",
			Help:      "dnsListRecords cache hits.",
		}),
		cacheMisses: prometheus.NewCounter(prometheus.CounterOpts{
			Namespace: "namesilo_webhook",
			Subsystem: "cache",
			Name:      "misses_total",
			Help:      "dnsListRecords cache misses (lists that reached the API).",
		}),
	}
...
// ObserveAPICall implements namesilo.Recorder.
